...
-- Function to calculate the maximum required width for each table column
...
local function getWindowDimensions(name)
    ...
    if name == "inbox" then
        ...
        -- Add 20 pixels for left/right window margin (10 on each side)
        w = totalContentWidth + 20 
        h = 450
    end
    return w, h
end
...
local function drawNetworkWindowContent(win)
    ...
    for itemName, data in pairs(marketData) do
        ...
        local nextDemand = data.nextDemand
...
function love.draw()
    ...
    if gameState == "selection" then
        ...
        love.graphics.setColor(0, 0, 0)
        love.graphics.line(
            StartButx, TaskBarStarty + StartButh + 7, 
            StartButx + StartButw, TaskBarStarty + StartButh + 7, 
